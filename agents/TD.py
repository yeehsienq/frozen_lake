...
def greedy(Q, s):
    ...
    def __init__(self, obs_shape, action_shape, lr=0.01, eps=0.1, gamma=0.95):

        # Initialize the Q matrix
        # Q: matrix r*c*A for row of grid, column of grid, and action taken at that state
        self.Q = np.zeros(obs_shape[0], obs_shape[1], action_shape) # row, column, action
...
class QLearning():
    ...
    def __init__(self, obs_shape, action_shape, train_log=100, num_episodes=10000, eval_freq=1000,
                 lr=0.01, eps=0.1, eps_decay=0, gamma=0.95):
        # Initialize the Q matrix
        # Q: matrix r*c*A for row of grid, column of grid, and action taken at that state
    ...
    def train(self, env):
        ...
        for i in range(self.num_episodes):
            ...
            while not done:
                ...
                # select action using e-greedy policy, and observe (s', reward) from action
                action = e_greedy(self.Q, state, self.eps)
                next_state, reward, done, info = env.step(action)

                # Q-learning Update
                self.Q[state[0]][state[1]][action] = self.Q[state[0]][state[1]][action] + self.lr * (
        ...
        plt.title("Q-Learning agent in frozen lake: evaluation rewards")
    ...
    def train(self, env):
        ...
        for i in range(self.num_episodes):
            ...
            while not done:
                ...
                # Choose next action from s' using e-greedy policy
                next_action = e_greedy(self.Q, next_state)
        ...
        plt.plot(x, rewards_total, label="training reward")
        plt.plot(x2, eval_rewards_total, label="eval reward")
        plt.title("SARSA agent in frozen lake: evaluation rewards")
        plt.legend(loc='lower right')
        plt.show()
